...
function App() {
  ...
  const handleClearChat = () => {
    if (window.confirm("Permanently wipe all conversation history from the vault? This cannot be undone.")) {
      setMessages([{
        id: 'welcome',
        role: 'model',
        text: `Good day, CEO. Tactical Core is active. How can I assist your operations today?`,
  ...
  const handleDeleteTip = (id: string) => {
    if (window.confirm("Remove this strategic tip from the vault?")) {
  ...
  const handleClearAllTips = () => {
    if (window.confirm("Purge all generated directives from the vault?")) {
                ...
                <div className="space-y-6 sm:space-y-8 animate-in fade-in delay-200">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                    ...
                    {savedTraining.length > 0 && (
                      <button 
                        onClick={() => { if(window.confirm("Purge curriculum history?")) setSavedTraining([]); }}
                        ...
                        <div key={module.id} className="bg-[#1b2537] border border-slate-800/50 rounded-3xl p-6 sm:p-8 shadow-xl hover:border-amber-500/30 transition-all group">
                          <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
                            ...
                            <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                              <button 
                                onClick={() => { if(window.confirm("Remove this module?")) setSavedTraining(prev => prev.filter(m => m.id !== module.id)); }}
